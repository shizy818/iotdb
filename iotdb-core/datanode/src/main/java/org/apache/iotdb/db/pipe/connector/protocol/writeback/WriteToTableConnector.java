...
import static org.apache.iotdb.commons.pipe.config.constant.PipeExtractorConstant.CONNECTOR_DATABASE_KEY;
import static org.apache.iotdb.commons.pipe.config.constant.PipeExtractorConstant.CONNECTOR_TABLE_KEY;
import static org.apache.iotdb.commons.pipe.config.constant.PipeExtractorConstant.SINK_DATABASE_KEY;
import static org.apache.iotdb.commons.pipe.config.constant.PipeExtractorConstant.SINK_TABLE_KEY;
...
@TableModel
public class WriteToTableConnector implements PipeConnector {
  private static final Logger LOGGER = LoggerFactory.getLogger(WriteToTableConnector.class);
  ...
  public void validate(PipeParameterValidator validator) throws Exception {
    ...
    // Check database name
    validator.validateSynonymAttributes(
        Collections.singletonList(CONNECTOR_DATABASE_KEY),
        Collections.singletonList(SINK_DATABASE_KEY),
        true);

    // Check table name
    validator.validateSynonymAttributes(
        Collections.singletonList(CONNECTOR_TABLE_KEY),
        Collections.singletonList(SINK_TABLE_KEY),
        true);
  }
  ...
  public void customize(PipeParameters parameters, PipeConnectorRuntimeConfiguration configuration)
    ...
    dataBaseName =
        parameters.getStringOrDefault(
            Arrays.asList(CONNECTOR_DATABASE_KEY, SINK_DATABASE_KEY), "testtdb");
    tableName =
        parameters.getStringOrDefault(Arrays.asList(CONNECTOR_TABLE_KEY, SINK_TABLE_KEY), "testtb");
    ...
    session =
        new InternalClientSession(
            String.format(
                "%s_%s_%s_%s",
                WriteToTableConnector.class.getSimpleName(),
  ...
  private void doTransferWrapper(
      ...
      throws PipeException, WALPipeException, IOException, IllegalPathException {
    // We increase the reference count for this event to determine if the event may be released.
    if (!pipeInsertNodeTabletInsertionEvent.increaseReferenceCount(
        WriteToTableConnector.class.getName())) {
      return;
    }
    try {
  ...
  private void doTransfer(final PipeRawTabletInsertionEvent pipeRawTabletInsertionEvent)
    ...
    insertTabletStatement.setDevicePath(new PartialPath(tableName));

    LOGGER.info(
        "insertTabletStatement.isWriteToTable : {}", insertTabletStatement.isWriteToTable());
    final TSStatus status = executeStatementForTableModel(insertTabletStatement, dataBaseName);
  ...
  private void doTransfer(
    ...
    insertBaseStatement.setDevicePath(new PartialPath(tableName));

    LOGGER.info("insertBaseStatement.isWriteToTable : {}", insertBaseStatement.isWriteToTable());
    final TSStatus status = executeStatementForTableModel(insertBaseStatement, dataBaseName);

    if (status.getCode() != TSStatusCode.REDIRECTION_RECOMMEND.getStatusCode()
        && status.getCode() != TSStatusCode.SUCCESS_STATUS.getStatusCode()) {
      throw new PipeException(
          String.format(
              "Write to table %s.%s PipeInsertNodeTabletInsertionEvent %s error, result status %s",
              dataBaseName, tableName, pipeInsertNodeTabletInsertionEvent, status));
    }
  }
