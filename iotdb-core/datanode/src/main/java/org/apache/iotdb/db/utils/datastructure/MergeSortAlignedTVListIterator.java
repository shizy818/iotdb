...
public class MergeSortAlignedTVListIterator implements IPointReader {
  ...
  private final int[] selectedTVList;
  ...
  public MergeSortAlignedTVListIterator(
    ...
    this.selectedTVList = new int[columnNum];
    this.validRowIndex = new int[columnNum];
      ...
      if (iterator.hasNext() && iterator.currentTime() <= time) {
        if (i == 0 || iterator.currentTime() < time) {
          for (int columnIndex = 0; columnIndex < columnNum; columnIndex++) {
            ...
            this.selectedTVList[columnIndex] = i;
            this.validRowIndex[columnIndex] = rowIndex;
  ...
  private TimeValuePair buildTimeValuePair() {
    ...
    for (int columnIndex = 0; columnIndex < vector.length; columnIndex++) {
      AlignedTVList.AlignedTVListIterator iterator =
          alignedTvListIterators[selectedTVList[columnIndex]];
      vector[columnIndex] = iterator.getPrimitiveObject(validRowIndex[columnIndex], columnIndex);
    }
  ...
  public TsPrimitiveType getPrimitiveObject(int iteratorIndex, int rowIndex, int columnIndex) {
    AlignedTVList.AlignedTVListIterator iterator = alignedTvListIterators[iteratorIndex];
    return iterator.getPrimitiveObject(rowIndex, columnIndex);
  }
